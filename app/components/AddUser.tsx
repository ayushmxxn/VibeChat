...
const AddUser: FunctionComponent<AddUserProps> = ({ onClose }) => {
    ...
    return (
        <div className={`p-6 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md shadow-lg border dark:border-slate-700 rounded-md absolute h-96 overflow-auto top-40 left-1 sm:left-72 m-auto w-96 ${!isDesktop && ''}`}>
            <form onSubmit={handleSearch} className="space-y-4 ">
                <input
                    type="text"
                    placeholder="Search"
                    name="username"
                    className="w-full dark:placeholder-slate-100 pl-3 bg-white/70  p-2 border dark:bg-slate-700/70 border-gray-300 dark:border-slate-700 text-sm rounded-lg focus:outline-none placeholder:text-sm font-normal"
                    autoComplete="off"
                    value={searchQuery}
                ...
                {loading ? (
                    ...
                    <ul className="mt-4 space-y-2">
                        {filteredUsers.map((user) => (
                            <li key={user.id} className="p-2  dark:bg-transparent overflow-auto  rounded flex items-center justify-between">
                                <div className="detail flex items-center space-x-4">
                                    <Image src={user.avatar || DefaultAvatar} alt="User Avatar" width={40} height={40} className="rounded-full" />
